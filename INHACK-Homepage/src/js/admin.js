...
async function initializeAdminPanel() {
  ...
  if (registerForm) {
    ...
    const adminRegIsAdminWrapper = document.getElementById('admin-reg-is-admin-wrapper');
    if (adminRegIsAdminWrapper) {
      adminRegIsAdminWrapper.style.display = currentUserIsSuperAdmin ? 'flex' : 'none';
    }
    ...
    try {
      ...
      users.forEach(user => {
        ...
        userRow.innerHTML = `
          ...
          <div class="user-col-activities" style="flex: 2; padding-right: 8px; position: relative; cursor: help; display: flex; align-items: center; overflow: visible;" data-tooltip="${user.activities || '없음'}">
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Strict admin path validation: immediately kick non-admins
  if (!me || !me.isAdmin) {
